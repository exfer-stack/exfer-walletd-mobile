...
const EN = {
  ...
  "home.depositBnbBody":
    "Send native BNB on the BNB Smart Chain (BSC) network to this address — do NOT use any other network (Beacon Chain / opBNB), or the funds can be lost. It's what you swap for EXFER and it pays the network fee. When withdrawing from an exchange, pick the BNB Smart Chain (BSC) network. Funds stay yours until you swap.",
  ...
  "swap.fundHint": "Send native BNB on the BNB Smart Chain (BSC) network to this address — do NOT use any other network (Beacon Chain / opBNB). It's both your funds and the network fee, no second token needed. Funds stay yours until you swap.",
...
const ZH: Record<MsgKey, string> = {
  ...
  "home.depositBnbBody":
    "请在 BNB 智能链（BSC）网络上往这个地址充入原生 BNB——千万不要用其他网络（Beacon Chain / opBNB），否则资金可能丢失。它就是你用来换 EXFER 的币，同时也用来付网络手续费。从交易所提币时，网络要选 BNB Smart Chain (BSC)。兑换前资金一直在你手中。",
  ...
  "swap.fundHint": "请在 BNB 智能链(BSC)网络上往这个地址充入原生 BNB——千万别用其他网络(Beacon Chain / opBNB)。它既是你的本金,也用来付网络手续费,无需第二种代币。兑换前资金一直在你手中。",
