...
const EN = {
  ...
  "mn.helpOk": "Got it",
  "mn.help1": "The phrase you wrote down never changes, and your address never changes on its own.",
  "mn.help2": "Different wallets derive keys from a phrase differently — the same words under a different method give a different address. So the phrase shown here can look different from the one you imported; that's normal, not an error.",
  ...
  "mn.techTitle": "Technical details",
  "mn.tech1": "Words: BIP-39 (24 English words).",
  "mn.tech2": "Standard: private key = SHA-256(\"EXFER-MNEMONIC-ED25519-V1\" + BIP-39 seed). Used by exfer.dev and the latest apps.",
  "mn.tech3": "Legacy: the 24 words' entropy is the private key directly (earlier addresses).",
  "mn.tech4": "Address = SHA-256(\"EXFER-ADDR\" + ed25519 public key), identical for both — so the same key always gives the same address.",
...
const ZH: Record<MsgKey, string> = {
  ...
  "mn.helpOk": "知道了",
  "mn.help1": "你记下的助记词永远不会变，你的地址也不会自己改变。",
  "mn.help2": "原因是不同钱包对助记词的派生算法不同——同一串词，算法不同就会算出不同地址。所以这里显示的有时会和你导入的那串看起来不一样，属于正常现象，不是出错。",
  ...
  "mn.techTitle": "技术细节",
  "mn.tech1": "词：BIP-39（24 个英文词）。",
  "mn.tech2": "标准方案：私钥 = SHA-256(\"EXFER-MNEMONIC-ED25519-V1\" + BIP-39 种子)，exfer.dev 与新版各端使用。",
  "mn.tech3": "旧版方案：把 24 个词的熵直接作为私钥（早期地址）。",
  "mn.tech4": "地址 = SHA-256(\"EXFER-ADDR\" + ed25519 公钥)，两种方案一致——所以私钥相同，地址必定相同。",
